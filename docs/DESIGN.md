...
- **Per-field locks** — every pickable field renders a small `FieldLock` toggle (`apps/web/src/components/wizard/FieldLock.tsx`). Default state: **free** (the fire treats the value as a suggestion it may revise). One tap toggles to **locked** (the fire must keep it). The default-free posture matches the philosophy of the wizards: every field is optional, and even when the player fills one in they're typically offering inspiration, not a hard requirement. Block-level locks exist for stats and spell selections; per-field locks exist for name / race / class / background / alignment / level. The Fireplace's `LockedField` composite (`apps/web/src/components/wizard/LockedField.tsx`) standardizes the input-plus-lock layout. **Level-up exception**: race / class / background / alignment / name are forced-locked by `patchedSheet()` regardless of the UI toggles — they're a structural invariant during a climb, not a default preference.
